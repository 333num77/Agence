...
import { ConfidenceScoreVO } from '../core/value-objects';
import { AsyncResult } from '../interfaces/core.interfaces';

export class DecisionAgent implements IDecisionAgent {
  
  async execute(query: DecisionQuery): AsyncResult<DecisionResult> {
    try {
  ...
  private analyzeEvidence(
    ...
    const evidenceTypes = new Set(evidence.map((e: any) => e.type));
    ...
    return {
      confidence: new ConfidenceScoreVO(confidenceValue, [{
  ...
  private generateViabilityRationale(evidence: any[], confidence: number): string {
    const positiveSignals = evidence.filter((e: any) => e.reliability > 0.7).length;
  ...
  private generateTechStackRationale(evidence: any[], confidence: number, constraints: any): string {
  ...
  private createDecision(
    ...
    let recommendation: RecommendationAction;
  ...
  private calculateUncertainty(risks: Risk[]): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
    ...
    if (highRisks > 0 || risks.filter(r => r.impact === 'MEDIUM').length > 2) return 'MEDIUM';
    return 'LOW';
  }
}
