 ...
 */
export interface IResearchResultRepository
  extends IRepository<IResearchResult, string> {
  findByProjectId(
    projectId: ProjectIdVO
  ): Promise<Result<IResearchResult[], Error>>;
  findByCategory(
    projectId: ProjectIdVO,
    category: string
  ): Promise<Result<IResearchResult[], Error>>;
  findLatestByProject(
    projectId: ProjectIdVO
  ): Promise<Result<IResearchResult | null, Error>>;
}
...
export interface IDecisionRepository extends IRepository<IDecision, string> {
  findByProjectId(
    projectId: ProjectIdVO
  ): Promise<Result<IDecision[], Error>>;
  findByType(
    projectId: ProjectIdVO,
    type: string
  ): Promise<Result<IDecision[], Error>>;
  findHighConfidenceDecisions(
    projectId: ProjectIdVO
  ): Promise<Result<IDecision[], Error>>;
  findLatestByProject(
    projectId: ProjectIdVO
  ): Promise<Result<IDecision | null, Error>>;
}
...
export interface IPlanRepository extends IRepository<IPlan, string> {
  ...
  findLatestByProject(
    projectId: ProjectIdVO
  ): Promise<Result<IPlan | null, Error>>;
}
