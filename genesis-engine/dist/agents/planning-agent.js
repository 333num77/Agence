...
class PlanningAgent {
    async execute(query) {
        try {
            ...
            for (const artifactType of query.artifacts) {
                const artifact = await this.generateArtifact(artifactType, query.decisions, query.constraints);
    ...
    async generateArtifact(type, decisions, constraints) {
        let content = '';
        let format = 'MARKDOWN';
        switch (type) {
            case 'PRD':
...
`;
    ...
    generateArchitecture(decisions, constraints) {
...
`;
    ...
    generateDatabaseSchema(decisions) {
        return `erDiagram
...
`;
    ...
    generateAPIContracts(decisions) {
        return `{
  "openapi": "3.0.0",
  "info": {
  ...
  "paths": {
    ...
    generateRoadmapDoc(decisions, constraints) {
...
`;
    ...
    generateDevelopmentPhases(decisions, constraints) {
...
`;
    ...
    generateAIExecutionPlan(decisions) {
...
`;
    ...
    generateSecurityPlan(decisions) {
...
`;
    ...
    generateTestStrategy(decisions) {
...
`;
    ...
    createRoadmap(decisions, constraints) {
    ...
    calculateCost(roadmap, constraints) {
